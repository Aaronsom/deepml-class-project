...
def transformer(max_len, vocab_len, mask_id=None, embedding=None, single_out=True,
                blocks=6, heads=8,  embedding_dim=300, input_sequence_length=None):
    inputs = Input(shape=(input_sequence_length, ))
    #TODO two inputs
    if mask_id is not None:
        masked_in = Masking(mask_value=mask_id)(inputs)
    else:
        masked_in = inputs
    if embedding is None:
        embedding = Embedding(input_dim=vocab_len, output_dim=embedding_dim)(masked_in)
    else:
        embedding = Embedding(input_dim=vocab_len, output_dim=embedding_dim, weights=[embedding])(masked_in)
    embedding = PositionalEncoding(max_len, embedding_dim)(embedding)
